...
#Ejercicio 1
```Scala
//Fórmula general del coeficiente de correlación de Pearson:
val datos = List(
  (72, 160),
  (76, 155),
  (78, 150),
...
// n equivale al numero total de observaciones en las listas de datos
val n = datos.length
// Media en (x)
val mediaenX = datos.map(_._1).sum.toDouble / n
// Media en (y)
val mediaenY = datos.map(_._2).sum.toDouble / n

// Numerador para la formula general
val numerador_ccp = datos.map(t => (t._1 - mediaenX) * (t._2 - mediaenY)).sum

val denominadorX = datos.map(t => Math.pow((t._1 - mediaenX),2)).sum
val denominadorY = datos.map(t => Math.pow((t._2 - mediaenY),2)).sum

// Denominador en X
//val denominadorX = Math.sqrt(datos.map(t => Math.pow(t._1 - mediaenX, 2)).sum)

// Denominador en Y
//val denominadorY = Math.sqrt(datos.map(t => Math.pow(t._2 - mediaenY,2)).sum)

val r = numerador_ccp / (Math.sqrt(denominadorX) * Math.sqrt(denominadorY))
```
#Ejercicio 2
```Scala
def at100(lists: List[List[Int]]): List[List[Int]] =
{
  // Con la variable  lists utilizo el filter para  ver que cumplan la condicion de la funcion anonima
  lists.filter(MeterN=>MeterN.max>=100)
}
//y muestra los elementos que sean mayores o igual a 100,  los recorre a todos los elementos
// Prueba de la función
val resultado = at100(List(List(0, 1, 100), List(60, 80), List(1000)))
